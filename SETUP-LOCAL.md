...
## Перезапуск сайта кафе

Только через скрипт, иначе получите белый экран:

```bash
bash ~/tools/cafe-restart.sh 3100
```

Он ищет владельца порта **по сокету**, гасит его, поднимает заново и проверяет,
что все файлы сборки отдаются с кодом 200.

## Грабли

- **Перезапуск «убей pid и запусти» ломает сайт.** Наблюдалось как
  `ChunkLoadError` и `500` на файлах `/_next/static/chunks/*` при живой главной
  странице. Причина цепочкой: в `logs/cafe.pid` попадает pid обёртки `npm`, а
  порт держит дочерний `next-server`; в его строке запуска **нет номера порта**,
  поэтому поиск по шаблону `3100` его не находит; новый процесс падает с
  `EADDRINUSE`, но старый продолжает отвечать **от прежней сборки** — браузер
  получает свежий HTML со ссылками на новые файлы, а сервер их не знает.
  Владельца порта искать по сокету: `ss -ltnp | grep :3100`.
